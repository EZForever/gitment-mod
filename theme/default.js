import { github as githubIcon, heart as heartIcon, spinner as spinnerIcon } from '../icons.js'
import { LANG, NOT_INITIALIZED_ERROR } from '../constants.js'
...
function renderHeader({ meta, user, reactions }, instance) {
  ...
  likeButton.innerHTML = `
    ...
    ${ meta.reactions && meta.reactions.heart
      ? ` • <strong>${meta.reactions.heart}</strong>${LANG.N_LIKED}`
      : ''
    }
  `
  ...
  commentsCount.innerHTML = `
    ${ meta.comments
    ? ` • <strong>${meta.comments}</strong>${LANG.N_COMMENTS}`
    : ''
    }
  `
  ...
  issueLink.href = meta.html_url || 'javascript:;'
  issueLink.target = '_blank'
  issueLink.innerText = LANG.ISSUE_PAGE
...
function renderComments({ meta, comments, commentReactions, currentPage, user, error }, instance) {
  ...
  if (error) {
    ...
    if (error === NOT_INITIALIZED_ERROR
      ...
      initButton.innerText = LANG.INITIALIZE_COMMENTS
    ...
    loading.innerText = LANG.LOADING_COMMENTS
    ...
    emptyBlock.innerText = LANG.NO_COMMENT_YET
  ...
  if (meta) {
    ...
    if (pageCount > 1) {
      ...
      if (currentPage > 1) {
        ...
        previousButton.innerText = LANG.PREVIOUS
      ...
      if (currentPage < pageCount) {
        ...
        nextButton.innerText = LANG.NEXT
...
function renderEditor({ user, error }, instance) {
  ...
  const disabledTip = user.login ? '' : LANG.LOGIN_TO_COMMENT
  container.innerHTML = `
      ${ user.login
        ...
        : user.isLoggingIn
          ...
          : `<a class="gitment-editor-avatar" href="${instance.loginLink}" title="${LANG.LOGIN_WITH_GITHUB}">
              ${githubIcon}
            </a>`
      }
    </a>
    <div class="gitment-editor-main">
      <div class="gitment-editor-header">
        <nav class="gitment-editor-tabs">
          <button class="gitment-editor-tab gitment-selected">${LANG.WRITE}</button>
          <button class="gitment-editor-tab">${LANG.PREVIEW}</button>
        </nav>
        <div class="gitment-editor-login">
          ${ user.login
            ? `<a class="gitment-editor-logout-link">${LANG.LOGOUT}</a>`
            : user.isLoggingIn
              ? LANG.LOGGING_IN
              : `<a class="gitment-editor-login-link" href="${instance.loginLink}">${LANG.LOGIN_WITH_GITHUB}</a>`
          }
        </div>
      </div>
      <div class="gitment-editor-body">
        <div class="gitment-editor-write-field">
          <textarea placeholder="${LANG.LEAVE_A_COMMENT}" title="${disabledTip}" ${shouldDisable}></textarea>
        </div>
    ...
    <div class="gitment-editor-footer">
      <a class="gitment-editor-footer-tip" href="https://guides.github.com/features/mastering-markdown/" target="_blank">
        ${LANG.MARKDOWN_SUPPORTED}
      </a>
      <button class="gitment-editor-submit" title="${disabledTip}" ${shouldDisable}>${LANG.COMMENT}</button>
    </div>
  `
  if (user.login) {
  ...
  previewTab.onclick = () => {
    ...
    if (!content) {
      preview.innerText = LANG.NOTHING_TO_PREVIEW
      return
    }

    preview.innerText = LANG.LOADING_PREVIEW
  ...
  submitButton.onclick = () => {
    submitButton.innerText = LANG.SUBMITTING
    ...
    instance.post(textarea.value.trim())
      .then(data => {
        ...
        submitButton.innerText = LANG.COMMENT
      })
      .catch(e => {
        alert(e)
        submitButton.removeAttribute('disabled')
